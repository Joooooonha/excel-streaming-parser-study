...
> [README](../README.md) · [의사결정 기록](investigation-log.md) · [실험 보고서](experiment-report.md) · [아키텍처](architecture.md)
...
- 실제 AWS 청구 비용 변화
- 안전한 최대 행·셀·출력 크기
- 다양한 `.xlsx` 유형의 Workbook 대비 출력 정합성

위 항목은 파서 구조를 선택한 연구 결론과 별개로, 실제 트래픽·배포 환경·제품 정책을 포함해야 하는 운영 검증 범위입니다.
...
현재 저장소의 수치는 인턴십 당시 연구 기록에서 옮긴 값입니다. 새 벤치마크를 구현할 경우 당시 결과를 덮어쓰지 않고 다음 디렉터리와 커밋에서 별도 관리합니다.
